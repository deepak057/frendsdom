...
if($_POST['n']>15)
$sql="select DATE_FORMAT(when1,'%d %M %Y'),UNIX_TIMESTAMP(when1),comment_index,fromid,comment from commentsonwebsite order by when1  LIMIT ".($_POST['n']-15).",{$_POST['n']}";
else
$sql="select DATE_FORMAT(when1,'%d %M %Y'),UNIX_TIMESTAMP(when1),comment_index,fromid,comment from commentsonwebsite order by when1 LIMIT 0,{$_POST['n']}";
...
$arg1='"commentsonwebsite","'.$row['comment_index'].'"';
...
<div style='width:540px;' class='comment_content'>
<div class='fl'>
<div class='image_part'>
<img src='".prof_pic($row['fromid'])."' align='middle'>
</div>
<div class='text_part'>
<a href='http://localhost/visit.php?id={$row['fromid']}' class='fback_from1' id='{$row['fromid']}'><b>".TuneTheName(user_name($row['fromid']),11).":</b></a>
</div>
<div class='clear'></div>
</div>
<div class='fr'>
...
echo "<tr id='postcomment_tr'><td><textarea name='txt_comment' style='width:450px; height:25px;' placeholder='Your comment' id='txt_comment' class='flexible_textarea'></textarea></td><td><input type='button' value='Comment' onclick='post_comment({$arg});'></td></tr><div style='position:relative;right:80px;top:-10px;'>";

//now showing and hiding previous ,next buttons
if($_POST['n']>15)echo "<img id='prev_comments' src='ngreyb.bmp' style='cursor:pointer;' title='Previous set of comments' onclick='get_profilecomments({$arg2});'>";if($_POST['n']<total_entries("commentsonwebsite","fromid",$comment_on_website))echo "<img id='next_comments' src='ngreyf.bmp' style='cursor:pointer;' title='Next set of comments' onclick='get_profilecomments({$arg1});'>";echo "</div>";
