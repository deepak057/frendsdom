...
{
		$time = strtotime($time);
    	$elapsed = time() - $time; // to get the time since that moment
    	$msPerMinute = 60;
	    ...
	    if($elapsed<=1000)
	    {
			$ago = 'just now';
	         	 ...
	         	 $ago =  round($elapsed/1000,0) + ' second ago'; 
	    }

	    else if ($elapsed < $msPerHour) {
	         	$ago =  round($elapsed/$msPerMinute) + ' minute ago';
	    }

	    else if ($elapsed < $msPerDay ) {
	    		$ago =  round($elapsed/$msPerHour,0) + ' hour ago';
	    }

	    else if ($elapsed < $msPerMonth) {
	    	if(round($elapsed/$msPerDay,0)>1)
	    	{
	    		$ago = round($elapsed/$msPerDay,0) + ' days ago'; 
	    	}
	    	else
	    	{
	    		$ago = round($elapsed/$msPerDay,0) + ' day ago'; 
	    	}
	    }

	    else if ($elapsed < $msPerYear) {
	    	if(round($elapsed/$msPerMonth)>1)
	    	{
	    		$ago = round($elapsed/$msPerMonth) + ' month ago';   
	    	}
	        else
	        {
	        	$ago = round($elapsed/$msPerMonth) + ' months ago';   
	        }
	    }

	    else {
	    	if(round(elapsed/msPerYear,0)>1)
	    	{
	    		$ago = round($elapsed/$msPerYear,0) + ' years ago';
	    	}
	         else{
	         	$ago = round($elapsed/$msPerYear,0) + ' year ago';
	         }  
	    }
	    return $ago;
}
...
/**
** Method to truncate the long texts and put a "..." only
**/

public static function TruncateTextWithoutReadMore($text,$chars=300,$class="c-gray pointer small hover_highlight"){
...
return $str1."<span class='read-more-btn ".$class."'>...</span><span class='none'>".$str2."</span>";
...
/*Get comment text*/
public static function GetCommentText($comment_id)
{
	$post = post_comments::model()->find("id=".$comment_id);
	if($post&&trim($post->comment_text)!="")
	{
		return self::TruncateTextWithoutReadMore($post->comment_text,30,"");	
	}
	else
	{
		return false;
...
/*Get post text*/
public static function GetPostText($post_id,$truncate=true)
{
	$post = posts::model()->find("id=".$post_id);
	if($post&&trim($post->supporting_text)!=""&&$truncate)
	{
		return self::TruncateTextWithoutReadMore($post->supporting_text,30,"");	
	}
	else if($post&&trim($post->supporting_text)!=""&&!$truncate)
	{
		return $post->supporting_text;
	}
	else
	{
		return false;
