...
/**
** Method to return the array of all the users associated with 
** given circle
**/

function AssociatedUsers($p_id=false){

$user_circles=user_circles::model()->findAll("circle_id=".$p_id." order by `date` asc");
...
/*
** Method to return array of circle objects of post
**/

function GetCircleName($post_id)
{
	$post = posts::model()->find('id ='.$post_id);
	if($post)
	{
		$circleId = $post->circle_id;
		$circle = circles::model()->find('id ='.$circleId);
		if($circle)
		{
			return $circle->title;
		}
...
/*
** Method to return array of circle id from post id
**/

function GetCircleId($post_id)
{
	$post = posts::model()->find('id ='.$post_id);
	if($post)
	{
