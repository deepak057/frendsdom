...
function actionRegister(){
	...
	if(!empty($_POST['email']) && !empty($_POST['password'])){
		...
		//manipulate mobile number
		$mobile=empty($_POST['mobile'])?"":$_POST['mobile'];

		$user_new=$user->Register($_POST['email'],$mobile,$_POST['password'],empty($_POST['no_auto_login']));
...
function actionRegisterSocial(){
		if(!empty($_POST['email'])){
			if($this->IsRegistered($_POST['email'])){
			
			$userController = helpers::get_controller(USERS); //get Users Controller object
			$user = Users::model()->find(array('condition'=>'email="'.$_POST['email'].'"'));
			$user->facebook_id = $_POST['id'];
			$user->save();
			$response_=$this->GetResponseArray(false,"A user already exists with this email",$user);
		...
		else
		{
			...
			$user_new=$user->RegisterSocial($_POST['email'],$_POST['name'],$_POST['profile_pic'],$_POST['password'],true,$_POST['id'],$_POST['website']);
			$response_=$this->GetResponseArray(true,"Successfully registered",$user_new);
		}
		$this->PrintResponse($response_);
	}
...
$notifications=$rt->RealTimeNotifications(!empty($_POST['last_notification'])?$_POST['last_notification']:0);
...
/**
** Method to update notification as read
**/

function actionHandleNotification(){
	if($_POST["action"]=="accept")
	{
		if(!empty($_POST["notification_id"]))
		{
			Helpers::get_controller(NOTIFICATION)->markAsRead($_POST["notification_id"]);
			$this->PrintResponse($this->GetResponseArray(true,"Markeda as read",array()));
		}
		else
		{
			$this->PrintResponse($this->GetResponseArray(false,"Something went wrong.",array()));
		}
	}
	else if($_POST["action"]=="all")
	{
		Helpers::get_controller(NOTIFICATION)->markAllAsRead();
		$this->PrintResponse($this->GetResponseArray(true,"All marked as read",array()));
	}		

}

/*Metho to return more notifications*/
public function  actionMoreNotifications(){
	$id = $_POST["last_id"];
	$notifications = Helpers::get_controller(NOTIFICATION)->GetPreviousNotifications("",$id);
	$return = [];
	...
	$this->PrintResponse($this->GetResponseArray(true,"More Notifications",array("notifications"=>$return)));
